...
PandemicCM PandemicCM::evolve() const {
  ...
  while (true) {
    ...
    for (int i = 0; i < current_it->get_social(); i++) {
      int r = uniform_1(gen);
      infect(next.population.begin() + r, next,
             eval_B(std::distance(population.begin(), current_it), r));
    }
    die_or_heal(next_it, next,
                eval_Y(std::distance(population.begin(), current_it)),
                eval_R(std::distance(population.begin(), current_it)));
  }
  return next;
}
      ...
      for (int j = 0; j < 2; j++) {
        if (population[i + j].get_state() == State::Infected) {
          for (int l = 0; l < 2; l++) {
            ...
            infect(next.population.begin() + index, next, eval_B(i + j, index));
          }
        }
      }
    }
    ...
    for (int i = 0; i < N; i = i + 10) {
      for (int j = 0; j < 10; j++) {
        if (population[i + j].get_state() == State::Infected) {
          ...
          die_or_heal(next.population.begin() + (i + j), next, eval_Y(i + j),
                      eval_R(i + j));
        }
      }
    }
