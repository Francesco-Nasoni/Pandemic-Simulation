...
double Graph::getMax_x() { return max_x; }

double Graph::getMax_y() { return max_y; }
...
void Graph::resize(double m_x, double m_y) {
  ...
  for (auto &v : point)
    for (long unsigned i = 0; i < v.getVertexCount(); i++) {
      ...
      v[i].position.y =
          (v[i].position.y - static_cast<double>(window.getSize().y) + 35) *
              y_ratio +
          static_cast<double>(window.getSize().y) - 35;
    }
  max_x = m_x;
  max_y = m_y;
}

void Graph::setColor(sf::Color c, int i) { color[i - 1] = c; }
...
void Graph::addPoint(sf::Vector2f n_point, int i) {
  ...
  n_point.y =
      static_cast<double>(window.getSize().y) - n_point.y * scale_y - 35;
