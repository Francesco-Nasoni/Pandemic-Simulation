...
Pandemic::Pandemic(int n, double b, double y, double r, double social, double i)
    : population(n, Person()), N{n}, B{b}, Y{y}, R{r}, m_social{social}
{
...
int Pandemic::get_susceptible() const
{
    return S;
}

int Pandemic::get_infected() const
{
    return I;
}

int Pandemic::get_dead() const
{
    return D;
}

int Pandemic::get_N() const
{
    return N;
}
...
Pandemic Pandemic::evolve()
{
    ...
    while (true)
    {
        ...
        {
            int r = uniform_1(gen);
            if (uniform_0(gen) < B && next.population[r].get_state() == State::Susceptible &&
                population[r].get_state() == State::Susceptible)
            {
