...
Axis::Axis(double max, double length, double width, int nDiv, sf::Color color, double x, double y, Direction d)
    : color{color}, x{x}, y{y}, max{max}, length{length}, nDiv{nDiv}, dir{d}
{
    ...
    for (int i = 0; i <= nDiv; i++)
    {
        ...
        else
        {
            ...
            lab_pos = sf::Vector2f(x - 10 - 3 * length - getOffSetY(str, 16.), -i * l + y + width - 8);
        }
